...
# make paths ----
#
path_gadm <- paste0(.get_path("onto", "_data"), "gadm36_levels.gpkg") # paste0(dir_data, "gadm_410-levels.gpkg")

# load data ----
#
# unpack the file, if it's not yet unpacked
if(!testFileExists(path_gadm)){
  if(!testFileExists(paste0(.get_path("onto", "_data"), "gadm36_levels_gpkg.zip"))){
    stop("please store 'gadm36_levels_gpkg.zip' in '", .get_path("onto", "_data"), "'")
  } else {
    message(" --> unpacking GADM basis")
    unzip(paste0(.get_path("onto", "_data"), "gadm36_levels_gpkg.zip"), exdir = .get_path("onto", "_data"))
  }
}
...
for(i in 1:6){
  ...
  # create a new mapping to the current labesl
...
# 4. write output ----
#
write_rds(x = temp_geo, file = paste0(.get_path("onto", "_data"), "geoscheme_to_gadm.rds"))
